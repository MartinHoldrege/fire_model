...
dfs_biome_long <- map(dfs_biome3, function(df) {
  ...
  df[, c(pred_vars, vars_nfire, vars_prop)] %>% 
...
dfs_deciles1 <- map(dfs_biome_long, function(df) {
  out <- df %>% 
    #.[1:1000, ] %>% # for testing
    # the named vector in select was selecting by the names
    # not the vector values!
    select(name, value, unname(vars_prop)) %>% 
    group_by(name) %>% 
    nest() %>% 
    # empirical cdf
    ...
    summarize(across(unname(vars_prop), mean),
...
decile_dotplot <- function(yvar, df, method) {
  ggplot(df, aes_string(x = 'mean_value', y = yvar)) +
  geom_point() +
  facet_wrap(~name, scales = 'free_x') +
  labs(x = "mean of decile of predictor variable",
       y = "mean observed fire probability (nfires/nyears)",
