...
subtitle: "Before/after fire, resampled/stratified data, hmod included"
...
Same modelling approach as used in the 05_models_biome-maks_fire-prob_byNFire.Rmd.

However, here an additional predictor variabl is added--
...
dfs_biome3 <- dfs_byNFire3_hmod %>% 
  # some rows have hmod missing, so removing those here
  map(drop_na)
...
```{r}
set.seed(1234)
pred_vars <- c("afgAGB", "pfgAGB", "MAT", "MAP", "prcpPropSum", 'hmod')
...
```{r}
n_categories <- 20 
# so resampled dataset is the same as before, not including hmod in the
# 'multidimensional' bins
df_pred_bin1 <- bin_df(df_pred, cols = pred_vars[pred_vars != 'hmod'],
...
```{r train, dependson='prep_data', cache = FALSE}
...
```{r pred_v_pred, cache=FALSE, dependson='train'}

cor(test_pred[pred_vars])
```
...
```{r response_ggpairs, cache=FALSE, dependson='train'}
map(method, function(x) {
...
g
jpeg("figures/quantile_plots/quantile_plot_hmod_v2.jpeg", units = "in", res = 600,
     width = 5, height = 3.5 )
g 
dev.off()
```
...
```{r}
g <- decile_dotplot_filtered_pq(df = pred_glm1_deciles_filt$paint)
g

jpeg("figures/quantile_plots/quantile_plot_filtered_hmod_v2.jpeg", units = "in", res = 600,
     width = 5.5, height = 6 )
g 
dev.off()
```
...
saveRDS(mods2save, 
        paste0("models/glm_binomial_models_byNFire_hmod_v2_bin",
               n_categories, "_cwf.RDS"))
