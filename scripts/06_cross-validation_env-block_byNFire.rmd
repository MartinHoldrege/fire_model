...
df_byNFire2_fold$yhat <- predict(mods1$paint_cwf, newdata = df_byNFire2_fold,
                                 type = 'response')
  
df_byNFire2_fold %>% 
  group_by(fold) %>% 
  summarise(observed_fire_probability = weighted.mean(cwf_prop, w = numYrs)*100,
            predicted_fire_probability = weighted.mean(yhat, w = numYrs)*100) %>% 
  select(fold, observed_fire_probability, predicted_fire_probability) %>% 
  kable(caption = "Mean observed/predicted fire probability by fold",
        digits = 2)

df_byNFire2_fold %>% 
  summarise(observed_fire_probability = weighted.mean(cwf_prop, w = numYrs)*100,
            predicted_fire_probability = weighted.mean(yhat, w = numYrs)*100) %>% 
  select(observed_fire_probability, predicted_fire_probability) %>% 
  kable(caption = "biome-wide Mean observed/predicted fire probability",
        digits = 2)
...
df_byNFire2_fold %>% 
  ...
  summarise(across(c(MAT, MAP, prcpPropSum, pfgAGB, afgAGB),
                .fns = list(mean = weighted.mean),
                w = numYrs)) %>% 
  ...
  mutate(across(matches("MAT|MAP"), round, digits = 1),
       ...
       across(matches('AGB'), round, digits = 1)) %>% 
...
df_byNFire2_fold %>% 
  ...
  mutate(across(matches("MAT|MAP"), round, digits = 1),
       across(matches("prcpPropSum"), round, digits = 3),
       across(matches('AGB'), round, digits = 1)) %>% 
  kable(caption = "biome-wide summary of climate variables")
```
...
jpeg("figures/pdp/cv_pdp_v1.jpeg", units = "in", res = 600,
     width = 6.8, height = 4)
  g2
dev.off()
g2
