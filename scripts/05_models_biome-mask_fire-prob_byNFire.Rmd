...
inset <- dfs_obs_v_pred$paint %>% 
  filter(variable == 'cwf') %>% 
  ggplot(aes(predicted*100, observed*100)) +
...
g2 <- decile_dotplot_filtered_pq2(df = pred_glm1_deciles_filt$paint,
                                  insets_left = list(A, C, E),
                                  insets_right = list(B, D, f))
g2
# add insets

jpeg("figures/quantile_plots/quantile_plot_filtered_insets_v1.jpeg", units = "in", res = 600,
     width = 7, height = 8 )
g2
dev.off()
