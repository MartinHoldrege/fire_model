...
inset <- dfs_obs_v_pred$paint %>% 
  filter(variable == 'cwf') %>% 
  ggplot(aes(observed*100, predicted*100)) +
...
g2 <- g +
  inset_element(inset, left = 0.7, bottom = -0.15,
                right = 1, top = 0.4)
g2
jpeg("figures/quantile_plots/quantile_plot_v3.jpeg", units = "in", res = 600,
     width = 5, height = 3.5 )
g2
dev.off()
```
...
```{r}
# create main figure
g <- decile_dotplot_filtered_pq2(df = pred_glm1_deciles_filt$paint)
```

Code to create obs vs pred insets isn't finished
...
left <- c(0.1, 0.5)
bottom <- c(0.2, 0.5, 0.8)
width <- 0.2

g +
  inset_element(inset_l1$afgAGB_MAT, 
                left = left[1],
                bottom = bottom[3],
                right = left[1] + width, 
                top = bottom[3] + width)

inset <- inset_l1$afgAGB_MAT
g +
  inset_element(inset,
                left = 0, bottom= 0.8, right = 0.25, top = 1,
                align_to = 'full')
```
...
```{r}
g
jpeg("figures/quantile_plots/quantile_plot_filtered_v3.jpeg", units = "in", res = 600,
     width = 7, height = 8 )
g 
dev.off()
```
...
jpeg("figures/quantile_plots/quantile_plot_filtered_mid_v1.jpeg", units = "in", res = 600,
     width = 5.5, height = 6 )
g 
dev.off()
```
