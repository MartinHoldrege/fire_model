...
library(tidyverse)
library(margins) # for cplot function
```
...
```{r}
hist(df_past3$fireProbYr, xlab = paste0(lab_fireProb[['fireProbYr']], 
                                        " (Pastick et al.)"),
     main = 'Data from all grid-cells')

hist(df_past3$fsim_bp, xlab = paste0(lab_fireProb[['fireProbYr']], 
                                        " (FSim data)"),
     main = 'Data from all grid-cells')
```

## comparing different fire probability data sources
...
cor(train$fireProbYr, train$fsim_bp) %>% 
  round(3)
```
...
mod_vars <- attributes(terms(gau_glm1))[["term.labels"]] %>% 
  str_subset(":", negate = TRUE) # not including interactions
...
```{r results='hide', fig.keep='all'}
# variables in the model
mod_vars <- attributes(terms(bin_glm1))[["term.labels"]] %>% 
  str_subset(":", negate = TRUE) # not including interactions

for (var in mod_vars){
  print(cplot(bin_glm1, var))
}
```
