...
# small dataset used for creating pdp plots (so code isn't too small)
train_pdp <- df_ann3 %>% 
  slice_sample(n = 1e5) %>% 
...
df_pdp_by_fold1 <- map_dfr(mods_train2, create_pdp_df, data = train_pdp,
...
df_pdp_mod_fold1 <- map_dfr(mods_test1, create_pdp_df, 
                                 data = train_pdp,
                   mod_vars = names(lookup_var),
                   .id = 'fold')
